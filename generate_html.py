...
<body class="min-h-screen flex flex-col antialiased selection:bg-brand-red selection:text-white">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full">
    ...
    <section id="tab-vision-mercado" class="space-y-6">
      ...
      <div class="glass-card p-4 rounded-xl border border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-2.5">
          ...
          <span class="text-[11px] font-mono text-slate-400">Base Técnica: <b>Total Primas y Recargos Unificados ($29.91 Billones)</b></span>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
        ...
        <div id="personasInsightBox" class="mt-2.5 p-2 bg-slate-900/80 rounded-lg border border-slate-800 text-[11px] text-slate-300 flex flex-wrap items-center justify-between gap-2">
          <div class="flex items-center gap-2">
            ...
            <span><b>Efecto Multirramo en Personas:</b> El <b>48.2% ($1.80 Billones)</b> de los productos de Personas y Vida es emitido por Aseguradoras Mixtas, y el <b>50.0% ($1.87 Billones)</b> por Aseguradoras exclusivas de Personas.</span>
          </div>
          <span class="font-mono text-rose-300 font-bold">Total Ramo Personas: $3.74 Billones (12.5% del mercado)</span>
        </div>
      </div>
          ...
          <table class="w-full text-left text-xs border-collapse table-auto">
            <thead class="text-slate-400 bg-slate-900/90 border-b border-slate-700 text-[11px]">
              <tr>
                ...
                <th class="py-2.5 px-2">Razón Social</th>
    ...
    <!-- ======================================================== -->
    <section id="tab-ramos-suscripcion" class="space-y-6">
      <div class="glass-card p-5 rounded-xl">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Barras = Primas Emitidas (ARS en Escala Argentina) | Línea y Etiquetas = Siniestralidad Devengada (%)</p>
          </div>
  ...
  <script>
    ...
    // Explicit Hispanic/Latin Financial Formatter
    // 10^12: Billones (escala larga, 1 millón de millones)
    // 10^9:  Mil Millones (1.000 millones)
    // 10^6:  Millones
    // 10^3:  Miles
    function formatARS(val) {{
      ...
      const sign = val < 0 ? '-' : '';
      if (absVal >= 1e12) {{
        return sign + '$' + (absVal / 1e12).toFixed(2) + ' Billones';
      }} else if (absVal >= 1e9) {{
        return sign + '$' + (absVal / 1e9).toFixed(2) + ' Mil Millones';
      }} else if (absVal >= 1e6) {{
        return sign + '$' + (absVal / 1e6).toFixed(2) + ' Millones';
      }} else if (absVal >= 1e3) {{
        return sign + '$' + (absVal / 1e3).toFixed(1) + ' Mil';
      }} else {{
    ...
    function formatARSCompact(val) {{
      ...
      if (absVal >= 1e12) {{
        ...
        return sign + '$' + (absVal / 1e9).toFixed(2) + ' Mil M.';
      }} else if (absVal >= 1e6) {{
        return sign + '$' + (absVal / 1e6).toFixed(2) + ' Millones';
      }} else {{
        return sign + '$' + absVal.toLocaleString('es-AR', {{ maximumFractionDigits: 0 }});
    ...
    function renderMarketOverview() {{
      ...
      let rowsHtml = top15.map((c, i) => {{
        ...
        <tr class="hover:bg-slate-800/60 ${{isHL ? 'bg-amber-500/20 border-l-4 border-l-amber-400 font-bold' : (isLS ? 'bg-amber-500/10 border-l-4 border-l-amber-400/70' : '')}} cursor-pointer" onclick="highlightScatterCompany('${{c.cod_cia}}')">
          ...
          <td class="py-2 px-2 font-semibold text-white truncate max-w-[220px]" title="${{c.razon_social}}">
            ${{c.razon_social}}
          ...
          <td class="py-2 px-2 text-right font-bold text-white">${{formatARSCompact(c.primas_emitidas)}}</td>
          <td class="py-2 px-2 text-right text-slate-300">${{formatARSCompact(c.primas_devengadas)}}</td>
          <td class="py-2 px-2 text-right text-rose-300">${{formatARSCompact(c.siniestros)}}</td>
          ...
          <td class="py-2 px-2 text-right ${{c.resultado_tecnico >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_tecnico)}}</td>
          <td class="py-2 px-2 text-right ${{c.resultado_financiero >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_financiero)}}</td>
          <td class="py-2 px-2 text-right font-bold ${{c.resultado_neto >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_neto)}}</td>
          <td class="py-2 px-2 text-right text-slate-300">${{formatARSCompact(c.activo)}}</td>
    ...
    function renderMarketTable() {{
      ...
      tbody.innerHTML = sorted.map(c => {{
        ...
        <tr class="hover:bg-slate-800/60 ${{isHL ? 'bg-amber-500/20 border-l-4 border-l-amber-400 font-bold' : ''}} cursor-pointer" onclick="highlightScatterCompany('${{c.cod_cia}}')">
          ...
          <td class="py-2 px-3 text-center">${{getTipoBadge(c.tipo_entidad)}}</td>
          <td class="py-2 px-3 text-right font-bold text-white">${{formatARSCompact(c.primas_emitidas)}}</td>
          <td class="py-2 px-3 text-right text-slate-300 font-bold">${{formatARSCompact(c.primas_devengadas)}}</td>
          <td class="py-2 px-3 text-right text-rose-300 font-bold">${{formatARSCompact(c.siniestros)}}</td>
          ...
          <td class="py-2 px-3 text-right ${{c.resultado_tecnico >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_tecnico)}}</td>
          <td class="py-2 px-3 text-right ${{c.resultado_financiero >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_financiero)}}</td>
          <td class="py-2 px-3 text-right font-bold ${{c.resultado_neto >= 0 ? 'text-emerald-400' : 'text-rose-400'}}">${{formatARSCompact(c.resultado_neto)}}</td>
          <td class="py-2 px-3 text-right text-slate-300">${{formatARSCompact(c.activo)}}</td>
    ...
    function renderRamosTab() {{
      ...
      // Custom Y-axis ticks in exact Spanish currency format (no English '50B')
      const tickCount = 5;
      const tickVals = [];
      const tickTexts = [];
      ...
      const tracePrimas = {{
        ...
        hovertemplate: '<b>%{{customdata.sub}}</b> (Cód: %{{customdata.cod}})<br>Primas Emitidas: <b>%{{customdata.primas_txt}}</b><br>Siniestros: <b>%{{customdata.sin_txt}}</b><br>Siniestralidad: <b>%{{customdata.loss_txt}}</b><extra></extra>'
      }};
      ...
      const traceLoss = {{
        ...
        hovertemplate: '<b>%{{x}}</b><br>Siniestralidad Devengada: <b>%{{y:.1f}}%</b><extra></extra>'
      }};
      ...
      const layout = {{
        ...
        yaxis2: {{
          title: 'Siniestralidad Devengada (%)',
          overlaying: 'y',
          side: 'right',
          color: '#E20039',
          ticksuffix: '%',
      ...
      // Subramos table with unambiguous full numbers
