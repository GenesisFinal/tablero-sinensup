...
<body class="min-h-screen flex flex-col antialiased selection:bg-brand-red selection:text-white">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1 w-full">
    ...
    <section id="tab-vision-mercado" class="space-y-6">
      ...
      <!-- 1. FULL WIDTH: Interactive Strategic Scatter Plot Matrix with Zoom & 4 Quadrants -->
      <div class="glass-card p-5 rounded-xl w-full">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p class="text-xs text-slate-400">Burbuja = Volumen de Primas Devengadas | Ejes centrados en 0% | Usa la rueda del ratón o selecciona un recuadro para hacer zoom</p>
          </div>
          
          <!-- Zoom & Reset Controls -->
          <div class="flex items-center gap-2">
            <button onclick="zoomScatterPlot(0.7)" title="Acercar Zoom" class="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-semibold text-slate-200 transition-colors flex items-center gap-1.5">
              <i class="fa-solid fa-magnifying-glass-plus text-brand-blue"></i> Zoom +
            </button>
            <button onclick="zoomScatterPlot(1.4)" title="Alejar Zoom" class="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-semibold text-slate-200 transition-colors flex items-center gap-1.5">
              <i class="fa-solid fa-magnifying-glass-minus text-brand-blue"></i> Zoom -
            </button>
            <button onclick="resetScatterPlotZoom()" title="Restablecer vista inicial" class="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-semibold text-slate-200 transition-colors flex items-center gap-1.5">
  ...
  <script>
    ...
    function renderMarketOverview() {{
      ...
      let rowsHtml = top15.map((c, i) => {{
        ...
        return `
        <tr class="hover:bg-slate-800/60 ${{isLS ? 'bg-amber-500/10 border-l-4 border-l-amber-400' : ''}}">
      ...
      if (extraToShow.length > 0) {{
        ...
        ` + extraToShow.map(c => {{
          ...
          return `
          <tr class="bg-amber-500/5 hover:bg-amber-500/15 border-l-4 border-l-amber-400">
